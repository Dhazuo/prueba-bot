<template>
    <div class="fixed w-full h-full p-4 flex justify-center font-press-start">
        <div class="p-4 w-2/4 relative container bg-white login opacity-0 transition duration-200 transform -translate-y-12 border border-gray-200 shadow-md">
            <div class="mx-auto text-center font-bold text-lg">Regístrate</div>
            <div class="mt-12 text-center mx-auto space-y-8">
                <div class="grid grid-cols-2 gap-x-4">
                    <div class="space-y-2 text-center">
                        ...
                        <div class="w-full">
                            <input v-model="form.name" id="name" type="text" class="text-center rounded-md shadow-sm border border-gray-100 focus:ring focus:ring-blue-500 transition duration-200 p-2 w-full">
                        </div>
                    </div>
                    <div class="space-y-2 text-center">
                        <label for="last_name">
                        ...
                        <div class="w-full">
                            <input v-model="form.last_name" id="last_name" type="text" class="text-center rounded-md shadow-sm border border-gray-100 focus:ring focus:ring-blue-500 transition duration-200 p-2 w-full">
                        </div>
                    </div>
                </div>
                <div class="space-y-2 text-center">
                ...
                <div class="flex mx-auto justify-center">
                    <button @click="register" :disabled="is_processing" :class="is_processing ? 'bg-gray-200 text-white hover:bg-gray-200' : 'bg-blue-500  hover:bg-blue-600'" class="mt-12 active:mt-14  relative transition duration-200 px-4 py-2 text-white">
                        ...
                        <div>
                            {{ is_processing ? 'Espera un momento' : 'Regístrate' }}
                        </div>
                    </button>
                </div>
            </div>
            <div class="absolute bottom-4 right-4 text-xs text-blue-500 underline">
                <button @click="$emit('login')">¿Ya tienes una cuenta? Inicia sesión.</button>
            </div>
        </div>
    </div>
</template>
